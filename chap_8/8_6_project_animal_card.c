...
void initAnimalArray(void); //배열에 모두 같은 수를 넣어주는 함수(카드의 뒷면은 모두 동일)
void initAnimalName(void); //선언한 배열(strAnimal)에 동물 이름 저장
void suffleAnimal(void);
int getEmptyPosition(void);
int conv_pos_x(int x);
int conv_pos_y(int y);
void printAnimals(void);
void printQuestion(void);
int foundAllAnimals(void); //모든 동물을 다 찾았는지 알아내는 (checkAnimal[x][y] ==0 (뒤집히지 않음)없는지 확인)
...
int main (void){
	...
	while(1) 
	{ //게임이 끝날 때 까지 카드 지도, 문제는 계속 보여줘야 함
...
int getEmptyPosition(void)
{ //빈공간 번호를 반환해줄거임. 이는 arrayAnimal 배열에 빈공간 없을때까지 작동하는 무한루프
...
void printQuestion(void)
{//번호를 골랐을 때 서로 일치하면 (앞면) 동물 이름을 표시, 아니면 그대로 (뒷면) 공간 번호 표시하기	
	printf("\n====================문제====================\n");
	...
	for (int i = 0; i < 4; i++)
	{
		for (int j = 0; j < 5; j++)
		{
			...
			{ //해당 위치의 카드 앞뒷면 확인하는 배열(전역변수), checkAnimal 배열의 요소 값이 1이면 짝을 맞춘 상태고 0이면 짝을 맞추지 못한 상태임.
